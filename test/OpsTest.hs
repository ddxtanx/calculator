...
intToRes :: (Integral a) => a -> FailRes
intToRes = pure . fromIntegral
...
testExp x y = evaluedVal == Result calcdVal
    where
        ...
        evaluedVal = eval $ Op Exp (Const x) (Const y)
        calcdVal = xf ** yf
...
testMod x y = evaluedVal == Result calcdVal
    where
        xf = fromIntegral x :: ResultType
        yf = fromIntegral y :: ResultType
        evaluedVal = eval $ Op Mod (Const x) (Const y)
